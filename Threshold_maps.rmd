...
For COMP3 the UK used 8 assessment areas, as in Charting Progress 2. Each country used their own assessment areas with no alignment.
...
For the previous UK comprehensive eutrophication assessment (COMP3) thresholds were the same for all assessment areas and only dependent on salinity with a different threshold for "coastal" (below 34.5psu) and "offshore" (above 34.5) waters. 
...
Other countries used different thresholds, resulting in a map of eutrophication results which is not coherent with waters changing between problem and non-problem areas

![COMP3 results](Shapefiles/COMP3_results.png)
...
leaflet(COMP4_areas) %>%
  addTiles() %>%
  addPolygons(color = "black", fillOpacity = 0, weight = 1, label = paste0(COMP4_areas$ID, ": ", COMP4_areas$Area_Name_), popup = COMP4_areas$Area_Name_) %>% # popups not quite working properly
  setView(-0,55,zoom = 5)
...
wfd <- sf::st_read("Shapefiles/WFD_UK/WFD_UK.shp", quiet = T)
wfd <- st_transform(wfd, CRS("+init=epsg:4326"))

#livbay

livbay_wfd <- st_crop(wfd, xmin = -5, ymin = 52, xmax = -2.5, ymax = 55)
...
livbay <- leaflet(COMP4_areas) %>%
  ...
  fitBounds(lng1 = bbox(subset(sf:::as_Spatial(st_zm(COMP4_areas)), ID == "LBPM"))[1], lat1 = bbox(subset(sf:::as_Spatial(st_zm(COMP4_areas)), ID == "LBPM"))[2], lng2 = bbox(subset(sf:::as_Spatial(st_zm(COMP4_areas)), ID == "LBPM"))[3], lat2 = bbox(subset(sf:::as_Spatial(st_zm(COMP4_areas)), ID == "LBPM"))[4])


#thames
thames_wfd <- st_crop(wfd, xmin = 0, ymin = 51, xmax = 3, ymax = 53)
...
thames <- leaflet(COMP4_areas) %>%
  ...
  fitBounds(lng1 = bbox(subset(sf:::as_Spatial(st_zm(COMP4_areas)), ID == "THPM"))[1], lat1 = bbox(subset(sf:::as_Spatial(st_zm(COMP4_areas)), ID == "THPM"))[2], lng2 = bbox(subset(sf:::as_Spatial(st_zm(COMP4_areas)), ID == "THPM"))[3], lat2 = bbox(subset(sf:::as_Spatial(st_zm(COMP4_areas)), ID == "THPM"))[4])


#humber
humber_wfd <- st_crop(wfd, xmin = -1, ymin = 52.5, xmax = 2, ymax = 54.5)
...
humber <- leaflet(COMP4_areas) %>%
  ...
  fitBounds(lng1 = bbox(subset(sf:::as_Spatial(st_zm(COMP4_areas)), ID == "HPM"))[1], lat1 = bbox(subset(sf:::as_Spatial(st_zm(COMP4_areas)), ID == "HPM"))[2], lng2 = bbox(subset(sf:::as_Spatial(st_zm(COMP4_areas)), ID == "HPM"))[3], lat2 = bbox(subset(sf:::as_Spatial(st_zm(COMP4_areas)), ID == "HPM"))[4])
...
THe E-hype project produced estimates of the DIN and DIP discharge for the major European rivers for the year 1900 corresponding to a pre industrialisation of production of condition.

### Eunosat
During the eunosat project a different approach for determining thresholds was developed. These were used to model "Reference conditions" for nutrients and chlorophyll for the year 1900 using E-hype inputs and 50% was added to these to form the assessment thresholds for nutrients and chlorophyll. This was based on the Deltares model.
...
The maps below present the HS1 thresholds, as for UK waters there is no difference between HS1 and HS2 scenarios. The UK ESG needs to agree where we can accept these and where they are not acceptable.
...
This table shows the HS1 thresholds for all assessment areas relevant to the UK.
```{r thresholds table}
COMP4_areas_COMP3_thres <- read.table("Shapefiles/COMP4_areas_COMP3_thres.csv", sep = ",", header = T)
UK_areas <- COMP4_areas_COMP3_thres[!is.na(COMP4_areas_COMP3_thres$COMP3_CPHL),]$ID
kable(thres[thres$ID %in% UK_areas, c(1:3, 6:8)])
```
...
In cases where the HS1 thresholds we should in the first instance look at the relative method thresholds. The relative method produces thresholds that are in most cases similar to the HS1 thresholds but sometimes disagree.

```{r relative method}
relmeth_thres <-read.table("Shapefiles/rel_method_thres.csv", sep = ",", header = T)
thres_methods2<- merge(thres[c(1:3,7)], relmeth_thres, by = "ID")
names(thres_methods2) <- gsub(x = names(thres_methods2), pattern = "x", replacement = "HS1")  
names(thres_methods2) <- gsub(x = names(thres_methods2), pattern = "y", replacement = "rel.method") 

thres_methods2 <- thres_methods2 %>% 
  filter(!is.na(thres_methods2$DIN.rel.method))
# p <- ggplot(thres_methods2, aes(x = DIN.rel.method, y = DIN.HS1)) +
#   geom_point(color = "blue") +
#   #geom_text(label = thres_methods2$ID, hjust=0, vjust=0) +
#   geom_abline()

#ggplotly(p)

lm <- lm(DIN.HS1 ~ DIN.rel.method, data = thres_methods2)
fig <- plot_ly(data = thres_methods2, x = ~DIN.rel.method, y = ~DIN.HS1, text= thres_methods2$ID) %>% add_markers(y = ~DIN.HS1) %>%  add_lines(x = ~DIN.rel.method, y = fitted(lm)) %>%
  layout(showlegend = F)
fig

```
For the some UK assessment areas, the thresholds from HS1 and the relative method differ by more than 20%. These include Thames plume, Humber plume and Northern North Sea for Chlorophyll, Liverpool Bay for DIN, and Southern North Sea for DIP. In these cases we may want to consider using the thresholds derived from the relative method instead.
```{r thres comparison}
plume_thres_methods2 <- thres_methods2[thres_methods2$ID %in% c("HPM", "LBPM", "THPM", "SNS", "NNS"),]
kable(plume_thres_methods2[c(1,2,5,3,6,4,7)])
```
...
```{r wfd}
wfd <- readOGR(dsn = "Shapefiles/WFD_UK", layer = "WFD_UK", verbose = F)
wfd <- spTransform(wfd, CRS("+init=epsg:4326"))
leaflet(wfd) %>%
  addTiles() %>%
  addPolygons(color = "green", fillOpacity = 0, weight = 1, label = paste0(wfd$WB_NAME, ";   Adjacent COMP4 assessment area: ", wfd$COMP4_area), highlight = highlightOptions(weight = 3, bringToFront = TRUE, sendToBack = TRUE)) %>% 
  addPolygons(data = COMP4_areas, color = "black", fillOpacity = 0, weight = 1, label = paste0(COMP4_areas$ID, " - ", COMP4_areas$Area_Name_), highlight = highlightOptions(weight = 3, bringToFront = TRUE, sendToBack = TRUE)) %>%
  setView(-0,55,zoom = 5)
...
Confidence ratings of the individual assessment parameters will be applied to indicate the reliability of the data gained from in-situ monitoring, monitoring using novel observation tools (automated buoys, satellite data) and modelling.  Confidence rating of novel observation tools and modelling is estimated during the processing of these data and provided together with the data products (work in progress), while the confidence of in-situ data is calculated directly in the COMPEAT tool.
COMPEAT calculates 3 types of confidence:
<li></li> Temporal confidence looks at the number of data points within a year, and the number of months represented.
<li></li> Spatial confidence looks at the proportion of each assessment area which has data using a grid
<li></li> Accuracy confidence uses confidence intervals to determine the confidence that an assessemnt result is above or below the threshold.

These 3 measures of confidence are combined into one overall confidence weighting for <em>in situ</em/> data

For the chlorophyll results where satellite data is also included, the confidence of the different data types is used to create a weighted average of <em>in situ</em/> and satellite data/ 

## Data

Perhaps for another document, include map of all data currently included in the assessment. talk about satellite data product.

## Assessment results

Below are the assessment results as created by the COMPEAT tool using the HS1 thresholds.

```{r assessment results}
#Load results

# Write results
wk3 <- read.table("Output_chl_weighted/2015-2020-HS1/Annual_Indicator.csv", sep = ",", header = T)
wk5 <- read.table("Output_chl_weighted/2015-2020-HS1/Assessment_Indicator.csv", sep = ",", header = T)
wk9 <- read.table("Output_chl_weighted/2015-2020-HS1/Assessment.csv", sep = ",", header = T)
...
wk <- merge(COMP4_areas, wk9, all.x = TRUE, by.x = "ID", by.y = "Code")
...
pal <- colorNumeric(palette = oceColorsChlorophyll(255), domain = c(0,20), na.color = "#00000000")
EQRSpal <- colorFactor(EQRS_Class_colors, levels = c("High", "Good", "Moderate", "Poor", "Bad"), na.color = "#00000000")

leaflet(wk) %>%
  addTiles() %>%
  #addPolygons(data = wk, fillColor = ~pal(chl_mean), color = "black", fillOpacity = 1, weight = 1, group = "Eutrophication State", popup = paste0("Mean chl = ", as.character(round(wk$chl_mean,2)), ";  No. data points = ", as.character(wk$chl_count))) %>%
  addPolygons(data = wk, fillColor = ~EQRSpal(EQRS_Class), color = "black", fillOpacity = 1, weight = 1, group = "EQRS", popup = wk$EQRS_Class) %>%
  #addLegend("bottomright", pal = pal, values = c(0,20), title = paste0("Chl (mg/l)"), group = "Eutrophication State") %>%
  ...
  #addLayersControl(overlayGroups = c("Eutrophication State", "EQRS"), position = c("bottomleft"), options = layersControlOptions(collapsed = FALSE, autoZIndex = TRUE)) %>%
  #hideGroup("EQRS") %>% 
        setView(3,55, zoom = 5)
      
  

# output$assessment_areas <- renderLeaflet({
#       EQRS_Class_colors <- c("#3BB300", "#99FF66", "#FFCABF", "#FF8066", "#FF0000")
#       EQRS_Class_limits <- c("High", "Good", "Moderate", "Poor", "Bad")
#       EQRS_Class_labels <- c(">= 0.8 - 1.0 (High)", ">= 0.6 - 0.8 (Good)", ">= 0.4 - 0.6 (Moderate)", ">= 0.2 - 0.4 (Poor)", ">= 0.0 - 0.2 (Bad)")
# 
#       COMP4_areas_update <- COMP4_areas_scenarios[[input$scenario]][[input$year]]
#       pal <- colorNumeric(palette = oceColorsChlorophyll(255), domain = c(input$range[1],input$range[2]), na.color = "#00000000")
#       EQRSpal <- colorFactor(EQRS_Class_colors, levels = c("High", "Good", "Moderate", "Poor", "Bad"), na.color = "#00000000")
#       
#       map = leaflet(COMP4_areas_update) %>%
#         addTiles() %>%
#         clearGroup("Eutrophication State") %>%
...
#         addPolygons(data = COMP4_areas_update, fillColor = ~EQRSpal(EQRS_Class), color = "black", fillOpacity = 1, weight = 1, group = "EQRS", popup = COMP4_areas_update$EQRS_Class) %>%
