...
DINdiff_map <- leaflet(COMP4_areas_COMP3_thres2) %>%
  addTiles() %>%
  addPolygons(fillColor = ~pal(DINmean_diff) , color = "black", fillOpacity = 1, weight = 1, label = paste0(COMP4_areas_COMP3_thres2$Area_Name_, ": DIN difference ", COMP4_areas_COMP3_thres2$DINmean_diff, " uM")) %>% 
  addLegend(pal = pal, values = c(-15,15), title = "DIN (uM)") %>%
  setView(-0,55,zoom = 5)

pal <- colorNumeric(palette = cmocean("curl")(255), domain = c(-15,15), na.color = "#00000000")
CPHLdiff_map <- leaflet(COMP4_areas_COMP3_thres2) %>%
  addTiles() %>%
  addPolygons(fillColor = ~pal(CPHL90_diff) , color = "black", fillOpacity = 1, weight = 1, label = paste0(COMP4_areas_COMP3_thres2$Area_Name_, ": CPHL ", COMP4_areas_COMP3_thres2$CPHL90_diff, " ug/l")) %>% 
  addLegend(pal = pal, values = c(-15,15), title = "Chlorophyll 90pc (ug/l)") %>%
  setView(-0,55,zoom = 5)
